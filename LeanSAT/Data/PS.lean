...
theorem varValue_fls_iff {σ : PS} {v : IVar} :
    (σ.varValue v = .fls) ↔ (PropFun.bind σ.toSubst ⟦v⟧) = ⟦.fls⟧ := by
  sorry
  done

theorem varValue_tr_iff {σ : PS} {v : IVar} :
    (σ.varValue v = .tr) ↔ (PropFun.bind σ.toSubst ⟦v⟧) = ⟦.tr⟧ := by
  sorry
  done
